...
bool DetermineGPSLocationForAddress (Address *address_p)
{
	...
	const json_t *geocoder_config_p = GetGlobalConfigValue ("geocoder");

	if (geocoder_config_p)
		{
			const char *value_s = GetJSONString (geocoder_config_p, "default_geocoder");

			if (value_s)
				{
					json_t *geocoders_p = json_object_get (geocoder_config_p, "geocoders");
					...
					if (geocoders_p)
						{
							...
							if (json_is_array (geocoders_p))
									...
									while (i < size)
										{
											...
											if (name_s && (strcmp (name_s, value_s) == 0))
												{
													geocoder_uri_s = GetJSONString (geocoder_p, "uri");
													i = size;
												}
											else
												{
							...
							else
								{
									...
									if (name_s && (strcmp (name_s, value_s) == 0))
										{
											geocoder_uri_s = GetJSONString (geocoders_p, "uri");
										}
								}

							if (geocoder_uri_s)
								{
									bool (*geocoder_fn) (Address *address_p, const char *uri_s) = NULL;

									if (strcmp (value_s, "google") == 0)
										{
											geocoder_fn = DetermineGPSLocationForAddressByGoogle;
										}
									else if (strcmp (value_s, "opencage") == 0)
										{
											geocoder_fn = DetermineGPSLocationForAddressByOpencage;
										}

									if (geocoder_fn)
										{
											success_flag = geocoder_fn (address_p, geocoder_uri_s);
										}
